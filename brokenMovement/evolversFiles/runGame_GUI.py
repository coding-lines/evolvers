        ...
        def onDrawFunction():
            if game.options.currentScreen == "game":
                if game.storage.playerInGame:
                    if not game.storage.cameraPos[0]%1 == game.storage.playerInformation["x"]%1:
                        game.storage.cameraPos[0] = int(game.storage.cameraPos[0])+game.storage.playerInformation["x"]%1
                        game.storage.cameraPos[2] = int(game.storage.cameraPos[2])+game.storage.playerInformation["x"]%1
                    if not game.storage.cameraPos[1]%1 == game.storage.playerInformation["y"]%1:
                        game.storage.cameraPos[1] = int(game.storage.cameraPos[1])+game.storage.playerInformation["y"]%1
                        game.storage.cameraPos[3] = int(game.storage.cameraPos[3])+game.storage.playerInformation["y"]%1
                mPos = list(pygame.mouse.get_pos())
                if not game.storage.cameraMoved:
                    game.storage.cameraSpeed = [0,0]
                else:
                    game.storage.cameraMoved -= 1
                if not game.storage.playerHasMoved:
                    game.storage.playerInformation["movementX"] = 0
                    game.storage.playerInformation["movementY"] = 0
                ...
                if game.storage.playerInGame:
                    ...
                    if game.storage.playerInformation["x"] > (game.storage.viewDistance/2):
                ...
                if game.options.currentScreen == "game":
                    ...
                    if game.options.currentScreen == "menu":
                        ...
                        if (mousePos[0] in range((game.options.dimensions[0]//2)-(startGame.get_rect().width//2),(game.options.dimensions[0]//2)+(startGame.get_rect().width//2)) and mousePos[1] in range(540,610)):
                            ...
                            game.timers.seconds = 0
                            game.storage.worldSize = [len(game.storage.gameWorld[0]),len(game.storage.gameWorld[0][0])]
                            game.storage.gameWorld = creatureEngine.runWorldIteration(game.storage.gameWorld)
                            game.storage.cameraPos = [1,1,34,22,1] if game.storage.viewDistance == 32 else [1,1,68,44,1] if game.storage.viewDistance == 64 else [1,1,18,12,1]
                            game.options.currentScreen = "game"
                    elif game.options.currentScreen == "newGame":
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if game.options.currentScreen == "game":
                ...
                if event.key == pygame.K_LEFT:
                    if not game.storage.playerInGame:
                        game.storage.cameraMoved = 2
                        game.storage.cameraSpeed[0] = (-(game.storage.camMaxSpeed)+game.storage.cameraSpeed[0]*19)/20
                ...
                if event.key == pygame.K_UP:
                    if not game.storage.playerInGame:
                        game.storage.cameraSpeed[1] = (-(game.storage.camMaxSpeed)+game.storage.cameraSpeed[1]*19)/20
                        game.storage.cameraMoved = 2
                ...
                if event.key == pygame.K_d:
                    if bool(game.timers.frame % 2) or game.storage.playerHasMoved == 0:
                        if int(game.storage.playerInformation["energy"]) > 0 and game.storage.playerInGame:
                            if not int(game.storage.playerInformation["x"]+1) >= game.storage.worldSize[0]:
                                game.storage.playerHasMoved = 4
                                game.storage.playerInformation["movementX"] = round((game.storage.playerInformation["movementX"]*19 + game.storage.playerInformation["attributes"]["speed"]/10) / 20,3)
                                game.storage.playerInformation["energy"] -= 1
                                game.storage.cameraPos = cameraEngine.moveCamera(game.storage.cameraPos,[game.storage.playerInformation["movementX"],0],game.storage.worldSize)
                                game.storage.playerInformation["x"] += game.storage.playerInformation["movementX"] if game.storage.playerInformation["x"] < game.storage.worldSize[0] else 0
                                game.storage.playerInformation["x"] = round(game.storage.playerInformation["x"],3)
                        else:
                            execute.evolvers.respawnPlayer()
                    ...
                    if bool(game.timers.frame % 2) or game.storage.playerHasMoved == 0:
                        if int(game.storage.playerInformation["energy"]) > 0 and game.storage.playerInGame:
                            if not int(game.storage.playerInformation["y"]+1) >= game.storage.worldSize[1]:
                                game.storage.playerHasMoved = 4
                                game.storage.playerInformation["energy"] -= 1
                                game.storage.playerInformation["movementY"] = round((game.storage.playerInformation["movementY"]*19 + game.storage.playerInformation["attributes"]["speed"]/10) / 20,3)
                                game.storage.cameraPos = cameraEngine.moveCamera(game.storage.cameraPos,[0,game.storage.playerInformation["movementY"]],game.storage.worldSize)
                                game.storage.playerInformation["y"] += game.storage.playerInformation["movementY"] if game.storage.playerInformation["y"] < game.storage.worldSize[0] else 0
                                game.storage.playerInformation["y"] = round(game.storage.playerInformation["y"],3)
                        else:
                            execute.evolvers.respawnPlayer()
